...
const (
	...
	// s3Scheme is the schema used on s3 StorageURLs
	s3Scheme string = "s3://"

	// s3Separator is the path separator for s3 StorageURLs
	s3Separator string = "/"
...
// StorageURL is the canonical representation of an object, either on local or remote
// storage.
...
type Option func(u *StorageURL)

func WithRaw(mode bool) Option {
	return func(u *StorageURL) {
		u.raw = mode
	}
}
...
func (u *StorageURL) Join(s string) *StorageURL {
	...
	clone := u.Clone()
	if !clone.IsRemote() {
		// StorageURL is local, thus clean the path by using path.Join which
		// removes adjacent slashes.
		clone.Path = path.Join(clone.Path, s)
		return clone
	}
	// StorageURL is remote, keep them as it is and join using string.Join which
	// allows to use adjacent slashes
	clone.Path = strings.Join([]string{clone.Path, s}, "")
	return clone
}

func (u *StorageURL) remoteStorageURL() string {
	s := u.Scheme + "://"
	if u.Bucket != "" {
		s += u.Bucket
	}
...
//	regex: ^a/b/c.*$
//	delimiter: "/"
func (u *StorageURL) setPrefixAndFilter() error {
	if u.raw {
		return nil
	}
